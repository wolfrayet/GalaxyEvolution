###Please note that the information of the Ha fit result is in "line_par". 
###First number is the flux, second is the central wavelength, the third one is the dispersion.
import numpy as np
...
def GaussianFitting(x, y, fr_min, fr_max, gn, peak_frac=5.):   #This function use curve_fit to fit the gussian function
    ...
    if gn==3:
        popt_gauss, pcov_gauss = curve_fit(function, x, y, p0=p0, \
                maxfev=100000,bounds=((0,fr_min,0,0,fr_min,0,0,fr_min,0),\
                (np.inf,fr_max,np.inf,np.inf,fr_max,np.inf,np.inf,fr_max,np.inf)))           
